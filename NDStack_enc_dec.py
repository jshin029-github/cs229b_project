...
import sys
sys.path.append('/home/jshin029/CS229B/NDStack/src')
...
# class encoder(jit.ScriptModule):
    ...
    def __init__(self, input_size, hidden_size, num_layers,
                 num_states, stack_alphabet_size,
                 batch_first=True):
        '''
        ...
        controller = lambda input_size: UnidirectionalLSTM(
        ...
        self.ndstack = NondeterministicStackRNN(
                input_size=input_size,
    ...
    def forward(self, x_input):
        ...
        # lstm_out, final = self.ndstack(x_input,
        #                                block_size = torch_semiring_einsum.AutomaticBlockSize())

        lstm_out = self.ndstack(x_input,
                                   block_size = torch_semiring_einsum.AutomaticBlockSize(),
                                   return_state=False,
...
# class decoder(jit.ScriptModule):
    ...
    def populate_hidden(self,encoder_hidden_states:(torch.Tensor,torch.Tensor)):
        self.hidden = encoder_hidden_states

    # @jit.script_method
    def forward(self, x_input:torch.Tensor,encoder_hidden_states:(torch.Tensor,torch.Tensor)):
        '''
        Forward pass for the LSTM
        Inputs:
            x_input:    input of shape (sequence_length, batch_size, input_size)
...
# class enc_dec(jit.ScriptModule):
    ...
    def __init__(self, input_size, output_size, hidden_size,
                ...
                batch_size=512, batch_first=True):
        '''
        ...
        self.encoder = encoder(input_size = input_size,
                               ...
                               num_states = num_states,
                               stack_alphabet_size = stack_alphabet_size)
        self.decoder = decoder(input_size = input_size+output_size + stack_alphabet_size*num_states,
    ...
    def train_model(self, input_tensor, target_tensor,
        ...
        n_batches = int(input_tensor.shape[self.batch_dim] / self.batch_size)
        ...
        for n_epoch in range(n_epochs):
            ...
            batch_idxs = torch.randperm(input_tensor.shape[0])

            for b in range(n_batches):
                if b % print_multiple == 0:
                ...
                # input_batch = input_tensor[batch_idxs[b:b+self.batch_size],:,:]
                # target_batch = target_tensor[batch_idxs[b:b+self.batch_size],:,:]
                ...
                last_encoder_hidden = encoder_outputs[0][:,-1:,:].transpose(0,1)

                # all_encoder_actions = [torch.cat([x[0].flatten(start_dim=1),
                #                                   x[1].flatten(start_dim=1),
                #                                   x[2].flatten(start_dim=1)],
                #                 axis=1) for x in encoder_outputs[1][2:]]

                # all_encoder_actions = [torch.zeros(
                #                         all_encoder_actions[0].shape).to(
                #                             all_encoder_actions[0].device)] + all_encoder_actions

                all_encoder_readings = encoder_outputs[1][1:]
                ...
                decoder_hidden = (last_encoder_hidden,torch.zeros(last_encoder_hidden.shape,device=input_tensor.device))

                # initialize first input for decoder
                decoder_output = torch.zeros(unraveled_targets[0].shape,
                                    device=input_tensor.device)

                # predict using mixed teacher forcing
                for t,(unraveled_input,unraveled_reading,unraveled_target) in enumerate(zip(unraveled_inputs,all_encoder_readings,unraveled_targets)):

                    # predict with teacher forcing

                    current_input = torch.cat([unraveled_input,
                                                unraveled_reading],
                                                axis=-1)
                    ...
                    if t == 0:
                        ...
                        decoder_input = torch.cat([decoder_output,
                                               current_input],axis=-1).unsqueeze(dim=self.t_dim)
                    ...
                    # self.decoder.module.populate_hidden(decoder_hidden)
                ...
                outputs = torch.stack(output_list,axis=self.t_dim)
    ...
    def predict(self, input_tensor, output_size):
        ...
        with torch.no_grad():
            ...
            unraveled_inputs = torch.unbind(input_tensor,dim=self.t_dim)

            decoder_hidden = (last_encoder_hidden,torch.zeros(last_encoder_hidden.shape,device=input_tensor.device))

            # initialize first input for decoder
            decoder_output = torch.zeros((input_tensor.shape[0],output_size),
                                device=input_tensor.device)

            for t,(unraveled_input,unraveled_reading) in enumerate(zip(unraveled_inputs,all_encoder_readings)):
